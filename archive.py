...
for root, dirs, files in walk(root_dir):
    ...
    if (manifest_name in files):
        ...
        logging.debug("Including paths {}".format(include_paths))

        for path in include_paths:
            rel_path = relpath(path, root)
            if rel_path in dirs:
                dirs.remove(rel_path)
                logging.debug("Ignoring dir {} as included for backup already".format(dir))
        
logging.info("Finished scanning directories")
logging.debug("All include paths: {}".format(paths))
